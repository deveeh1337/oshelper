                ...
                downloadUrlToFile(updatelink, thisScript().path,
                  function(id3, status1, p13, p23)
                    ...
                    elseif status1 == dlstatus.STATUS_ENDDOWNLOADDATA then
                      msg('Ñêðèïò óñïåøíî îáíîâèëñÿ äî âåðñèè '..updateversion..'.')
                      updatestatus = false
                      goupdatestatus = true
                    ...
                    if status1 == dlstatus.STATUSEX_ENDDOWNLOAD then
                      if goupdatestatus == nil then
                        msg('Íå ïîëó÷àåòñÿ îáíîâèòüñÿ, çàïóñêàþ ñòàðóþ âåðñèþ ('..thisScript().name..')')
                        update = false
                      end
                    end
                  end
...
function imgui.BeforeDrawFrame()
	...
	if fontsize == nil then
        fontsize = imgui.GetIO().Fonts:AddFontFromFileTTF(getFolderPath(0x14) .. '\\trebucbd.ttf', 16.0, nil, imgui.GetIO().Fonts:GetGlyphRangesCyrillic()) -- âìåñòî 30 ëþáîé íóæíûé ðàçìåð
    end
end
...
function main()
    ...
    sampRegisterChatCommand('pr', function()
		if prmanager.v then pronoroff = not pronoroff; msg(pronoroff and 'Ðåêëàìà âêëþ÷åíà.' or 'Ðåêëàìà âûêëþ÷åíà.') end
		...
		sampRegisterChatCommand('fin', function(arg)
			if cmds.v then 
			    if arg:find('(%d+) (%d+)') then
			        arg1, arg2 = arg:match('(.+) (.+)')
			        sampSendChat('/showbizinfo '..arg1..' '..arg2) -- 2+ àðãóìåíòà
			    else
			        msg('/fin [id èãðîêà] [id áèçíåñà]', -1)
			    end
			end
		end)
    ...
    while true do
        ...
        if calctext:find('%d+%%%*%d+') then
            ...
            result = textcolor..'Ðåçóëüòàò: '..color..number
        end
        if calctext:find('%d+%%%/%d+') then
            number1, number2 = calctext:match('(%d+)%%%/(%d+)')
        	...
        	if spawn.v and wasKeyPressed(0x04) then 
	     	...
	     	if armor.v and isKeyDown(0x12) and wasKeyPressed(0x31) then
	     		...
	     		if armourlvl > 89 then 
		     		msg('Ó âàñ '..armourlvl..' ïðîöåíòîâ áðîíè.')
    ...
    if window.v then
        ...
        imgui.Begin('OS Helper | '..thisScript().version, window, imgui.WindowFlags.NoResize)
	        imgui.BeginChild("left", imgui.ImVec2(150, 290), true)
				if imgui.Selectable(fa.ICON_FA_USER..u8' Ïåðñîíàæ', menu == 1) then menu = 1
				elseif imgui.Selectable(fa.ICON_FA_CAR..u8' Òðàíñïîðò', menu == 2) then menu = 2
				elseif imgui.Selectable(fa.ICON_FA_USERS..u8' Ñåìüÿ', menu == 3) then menu = 3
				elseif imgui.Selectable(fa.ICON_FA_GLOBE..u8' Îêðóæåíèå', menu == 8) then menu = 8
				elseif imgui.Selectable(fa.ICON_FA_COMMENTS..u8' Ðàáîòà ñ ÷àòîì', menu == 4) then menu = 4
				elseif imgui.Selectable(fa.ICON_FA_WINDOW_MAXIMIZE..u8' Ðàáîòà ñ äèàëîãàìè', menu == 5) then menu = 5
				elseif imgui.Selectable(fa.ICON_FA_COG..u8' Íàñòðîéêè', menu == 6) then menu = 6
				elseif imgui.Selectable(fa.ICON_FA_INFO_CIRCLE..u8' Èíôîðìàöèÿ', menu == 7) then menu = 7
				end
				imgui.SetCursorPosY(265)
				lua_thread.create(function()
					if updatestatus then
				        	if imgui.Button(u8'Îáíîâèòü', imgui.ImVec2(135, 20)) then
				        		imgui.ShowCursor = false
					          autoupdate("https://raw.githubusercontent.com/deveeh/oshelper/master/update.json", '['..string.upper(thisScript().name)..']: ', "")
				        	end
			    else 
			    	if imgui.Button(u8'Ñîõðàíèòü', imgui.ImVec2(135, 20)) then
			        		save()
									msg('Âñå íàñòðîéêè ñîõðàíåíû.')
			        end
			    end
			  end)
			imgui.EndChild()
			...
			if menu == 1 then
				imgui.PushFont(fontsize)
        			imgui.CenterText(u8'Ïåðñîíàæ')
        		imgui.PopFont()
        		imgui.Separator()
        		if imgui.Checkbox(u8'Áðîíåæèëåò', armor) then cfg.settings.armor = armor.v end
				imgui.TextQuestion(u8'Èñïîëüçîâàòü áðîíåæèëåò: ALT + 1')
				if imgui.Checkbox(u8'Ìàñêà', mask) then cfg.settings.mask = mask.v end
				imgui.TextQuestion(u8'Èñïîëüçîâàòü ìàñêó: ALT + 2')
				if imgui.Checkbox(u8'Íàðêîòèêè (3 øò)', drugs) then cfg.settings.drugs = drugs.v end
				imgui.TextQuestion(u8'Èñïîëüçîâàòü íàðêî: ALT + 3')
				if imgui.Checkbox(u8'Àïòå÷êà', med) then cfg.settings.med = med.v end
				imgui.TextQuestion(u8'Èñïîëüçîâàòü àïòå÷êó: ALT + 4')
			end
			if menu == 2 then
				imgui.PushFont(fontsize)
        			imgui.CenterText(u8'Òðàíñïîðò')
        		imgui.PopFont()
        		imgui.Separator()
				if imgui.Checkbox(u8'AutoCar', autolock) then cfg.settings.autolock = autolock.v end
				imgui.TextQuestion(u8'Àêòèâàöèÿ: ñåñòü â ìàøèíó\nÀâòîìàòè÷åñêîå çàêðûòèå äâåðåé + âêëþ÷åíèå äâèãàòåëÿ')
				if imgui.Checkbox(u8'Îòêðûòü/Çàêðûòü äâåðè', lock) then cfg.settings.lock = lock.v end
				imgui.TextQuestion(u8'Àêòèâàöèÿ: L')
				if imgui.Checkbox(u8'Ðåìêîìïëåêò', rem) then cfg.settings.rem = fill.v end
				imgui.TextQuestion(u8'Èñïîëüçîâàòü ðåìêîìïëåêò: R')
				if imgui.Checkbox(u8'Êàíèñòðà', fill) then cfg.settings.fill = fill.v end
				imgui.TextQuestion(u8'Èñïîëüçîâàòü êàíèñòðó: ALT + R')
				if imgui.Checkbox(u8'Ñïàâí òðàíñïîðòà', spawn) then cfg.settings.spawn = spawn.v end
				imgui.TextQuestion(u8'Èñïîëüçîâàíèå: Êîëåñèêî Ìûøè (íàæàòèå)')
				if imgui.Checkbox(u8'+W moto/bike', plusw) then cfg.settings.plusw = plusw.v end
				imgui.TextQuestion(u8'Èñïîëüçîâàíèå: W (çàæàòèå)\nÊëèêåð äëÿ âåëîñèïåäîâ è ìîòîöèêëîâ')

			end
			if menu == 3 then
				imgui.PushFont(fontsize)
        			imgui.CenterText(u8'Ñåìüÿ')
        		imgui.PopFont()
				imgui.Separator()
				if imgui.Checkbox(u8'Ìåíþ ñåìüè', fmenu) then cfg.settings.fmenu = fmenu.v end
				imgui.TextQuestion(u8'Àêòèâàöèÿ: O')
				if imgui.Checkbox(u8'Èíâàéò â ñåìüþ', finv) then cfg.settings.finv = finv.v end
				imgui.TextQuestion(u8'Àêòèâàöèÿ: F + 1')
			end
			if menu == 4 then
				imgui.PushFont(fontsize)
        			imgui.CenterText(u8'Ðàáîòà ñ ÷àòîì')
        		imgui.PopFont()
				imgui.Separator()
				if imgui.Checkbox(u8'Chat Helper', chathelper) then cfg.settings.chathelper = chathelper.v end
				imgui.TextQuestion(u8'Ïîäñêàçêè â ÷àòå')
				if imgui.Checkbox(u8'Chat Calculator', calcbox) then cfg.settings.calcbox = calcbox.v end
				imgui.TextQuestion(u8'Àêòèâàöèÿ: 1+1 (â ÷àò)')
				if imgui.Checkbox(u8'PR Manager', prmanager) then cfg.settings.prmanager = prmanager.v end
				imgui.TextQuestion(u8'Ìåíþ: /prm')
				if imgui.Checkbox(u8'Ñîêðàùåííûå êîìàíäû', cmds) then cfg.settings.cmds = cmds.v save() end
				if imgui.IsItemHovered() then
                    imgui.BeginTooltip()
			...
			if menu == 5 then
				imgui.PushFont(fontsize)
        			imgui.CenterText(u8'Ðàáîòà ñ äèàëîãàìè')
        		imgui.PopFont()
        			imgui.CenterText(u8'Íå ðàáîòàåò ñ íîâûìè äèàëîãàìè!')
				imgui.Separator()
				if imgui.Checkbox(u8'Àâòîëîãèí â áàíêå', cardlogin) then cfg.settings.cardlogin = cardlogin.v end
				if cardlogin.v then 
				imgui.Text(u8'Ïèí-êîä:')
				imgui.SameLine()
				imgui.PushItemWidth(54.5) 
				if imgui.InputInt(u8'##ëîãèí áàíê', logincard, 0, 0) then cfg.settings.logincard = logincard.v end
				end
				--imgui.TextQuestion(u8'Àêòèâàöèÿ: ALT + F')
			end
			if menu == 6 then
				imgui.PushFont(fontsize)
        		...
        		imgui.PopFont()
				imgui.Separator()
				imgui.offset(u8'Àêòèâàöèÿ ìåíþ: ') 
			if imgui.Combo(u8'##Àêòèâàöèÿ', active, {u8'Êîìàíäà', u8'×èò-êîä'}, -1) then cfg.settings.active = active.v save() end
			if imgui.IsItemHovered() then
	            imgui.BeginTooltip()
	                imgui.Text(u8'Ïîñëå èçìåíåíèÿ ðåæèìà àêòèâàöèè, ñîõðàíèòå è ïåðåçàãðóçèòå ñêðèïò.')
	            imgui.EndTooltip()
            end
				if active.v == 1 then
					imgui.offset(u8' ×èò-êîä: ')
					--if imgui.InputText(u8'##×èò-êîä', cheatcode) then cfg.settings.cheatcode = cheatcode.v save() end
					if imgui.InputTextWithHint(u8"##×èò Êîä", cfg.settings.cheatcode, cheatcode) then cfg.settings.cheatcode = cheatcode.v end
				end
				--if cheatcode.v == '' then cheatcode.v = 'oh' cfg.settings.cheatcode = 'oh' end
				imgui.offset(u8'Öâåò òåìû: ') 
					if imgui.Combo(u8'##Òåìà', theme, {u8'Êðàñíûé', u8'Çåëåíûé', u8'Æåëòûé'}, -1) then cfg.settings.theme = theme.v save()
					if cfg.settings.theme == 0 then themeSettings(1) color = '{ff4747}'
			...
			if menu == 7 then
        		...
        		imgui.PopFont()
				imgui.Separator()
				imgui.Text(fa.ICON_FA_ADDRESS_CARD..u8' Àâòîðû:') imgui.SameLine() imgui.Link('https://vk.com/deveeh', 'deveeh') imgui.SameLine() imgui.Text(u8'è') imgui.SameLine() imgui.Link('https://t.me/atimohov', 'casparo')
			end
			if menu == 8 then
				imgui.PushFont(fontsize)
        			imgui.CenterText(u8'Îêðóæåíèå')
        		imgui.PopFont()
				imgui.Separator()
				if imgui.Checkbox(u8'Ðåäàêòîð âðåìåíè è ïîãîäû', timeweather) then cfg.settings.timeweather = timeweather.v end
				if timeweather.v then
    ...
    if prmwindow.v then
    	...
    	imgui.Begin('OS Helper | '..thisScript().version..'##prmenu', prmwindow, imgui.WindowFlags.NoResize)
    		imgui.PushFont(fontsize)
        			...
        			imgui.CenterText(u8'Àêòèâàöèÿ /pr')
        	imgui.PopFont()
        	imgui.Separator()
        	if prmanager.v then
	        	if imgui.Checkbox(u8'Ðåêëàìà â VIP CHAT (/vr)', vr1) then cfg.settings.vr1 = vr1.v end
				if vr1.v then
					imgui.Text(u8'Ñîîáùåíèå: ')
					imgui.SameLine()
					if imgui.InputTextWithHint(u8"##vr1", u8"Ðàáîòàåò ÁÊ Ëûòêàðèíî ¹56!", vrmsg1) then cfg.settings.vrmsg1 = vrmsg1.v end
					end
				if imgui.Checkbox(u8'Ðåêëàìà â FAMILY CHAT (/fam)', fam) then cfg.settings.fam = fam.v end
				if fam.v then
					imgui.Text(u8'Ñîîáùåíèå: ')
					imgui.SameLine()
					if imgui.InputTextWithHint(u8"##fammsg", u8"Ðàáîòàåò Ëîìáàðä ¹240!", fammsg) then cfg.settings.fammsg = fammsg.v end
					end
				if imgui.Checkbox(u8'Ðåêëàìà â ALLIANCE CHAT (/al)', al) then cfg.settings.al = al.v end
				if al.v then
					imgui.Text(u8'Ñîîáùåíèå: ')
					imgui.SameLine()
					if imgui.InputTextWithHint(u8"##almsg", u8"Ðàáîòàåò ÁÊ Ëûòêàðèíî ¹56!", almsg) then cfg.settings.almsg = almsg.v end
					end
				if imgui.Checkbox(u8'Ðåêëàìà â AD (/ad 1)', adbox) then cfg.settings.adbox = adbox.v end
				if adbox.v then
					imgui.Text(u8'Ñîîáùåíèå: ')
					imgui.SameLine()
					if imgui.InputTextWithHint(u8"##admsg1", u8"Ðàáîòàåò ÁÊ Ëûòêàðèíî ¹56!", admsg1) then cfg.settings.admsg1 = admsg1.v end
					end
				if imgui.Checkbox(u8'Äîïîëíèòåëüíàÿ ñòðîêà', prstring) then cfg.settings.prstring = prstring.v end
				if prstring.v then
					imgui.Text(u8'Ñîîáùåíèå: ')
					imgui.SameLine()
					if imgui.InputTextWithHint(u8"##prstring", u8"/vr Ðàáîòàåò Ëîìáàðä ¹240!", stringmsg) then cfg.settings.stringmsg = stringmsg.v end
					end
				imgui.Separator()
				imgui.Text(u8'					Çàäåðæêà: ')
				imgui.SameLine()
				imgui.PushItemWidth(40)
				if imgui.InputInt("##Çàäåðæêà", delay, 0, 0) then cfg.settings.delay = delay.v end
				imgui.SameLine() 
				imgui.Text(u8'ñåê.')			
		    else
		    	imgui.CenterText(u8'Âêëþ÷èòå â ãëàâíîì ìåíþ ôóíêöèþ PR Manager.')
		    end
		    imgui.SetCursorPos(imgui.ImVec2(5, 375))
		    if imgui.Button(u8'Ñîõðàíèòü', imgui.ImVec2(290, 20)) then
		        save()
		        msg('Âñå íàñòðîéêè ñîõðàíåíû.')
		    end
